...
parser.add_argument('--tcn', type=int, default=10,
                    help='target class number')
parser.add_argument('--mul', type=int, default=1,
                    help='mutliplier')
parser.add_argument('--printstatus', action='store_true', default=False, help='print status')
...
def get_ddp_model(model, device, gpu):
    model = model.to(device)

    ddp_model = DistributedDataParallel(model, device_ids=[gpu], find_unused_parameters=True)
...
def main():
    global args, best_err1, best_err5, numberofclass, world_size
    ...
    is_rank_zero = gpu == 0
    allow_print_to_master(is_rank_zero)
    torch.cuda.set_device(gpu)
    device = torch.device(f'cuda:{gpu}')

    if is_rank_zero:
        args.exp_name = f'resnet_50_{args.tcn}_{args.mul}'

        args.start_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        if args.use_wandb:
            wandb.init(project=args.project_name, name=args.exp_name, config=args, reinit=True)
    ...
    if args.dataset.startswith('cifar'):
        ...
        if args.dataset == 'cifar100':
            ...
            train_dataset = Subset(train_dataset, [x for x in range(traindataperclass * args.tcn)])

            dupval = int(100/args.tcn) * args.mul
            new_train_dataset = train_dataset
            for _ in range(dupval-1):
                new_train_dataset += train_dataset
    ...
    model = resnet.resnet50_tutorial()
    model, ddp_model = get_ddp_model(model, device, gpu)
    ...
    if is_rank_zero and args.use_wandb:
        ...
        throughput = float(flops.split(' ')[0]) / (throughput_end - throughput_start)

        wandb.config.throughput = throughput
        wandb.config.params = params
        wandb.config.flops = flops
    ...
    for epoch in range(0, args.epochs):
        ...
        # train for one epoch
        train_loss = train(train_loader, ddp_model, criterion, optimizer, epoch)

        # evaluate on validation set
        err1, err5, val_loss = validate(val_loader, ddp_model, criterion, epoch)
        ...
        if is_rank_zero:
            ...
            if args.use_wandb:
                wandb.log({
                    ...
                    'val_loss': val_loss,
                    'top1': numberofclass - err1,
                    'top5': numberofclass - err5,
                    'top1_best': numberofclass - best_err1,
                    'top5_best': numberofclass - best_err5,
                })
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        printstatus(target, output)

        loss = reduce_mean(loss, world_size)
