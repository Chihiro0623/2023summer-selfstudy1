...
def get_ddp_model(model, args):
    ...
    if args.distributed:
        ddp_model = DistributedDataParallel(model, device_ids=[args.gpu], find_unused_parameters=True)
    else:
        ddp_model = None
...
def resume_from_checkpoint(checkpoint_path, model, optimizer=None, scaler=None, scheduler=None, iter_per_epoch=None, teacher=False):
